...
async function createSelfHostedPlatform(config: PlatformConfig): Promise<Platform> {
  ...
  // manage their own host's bare OpenResty; a REMOTE box's container edge is
  // resolved by probing that box via createInfraProvider instead — not from
  // this env, which describes the control plane and says nothing about the
  // target). Gated on the local case so remote routing is untouched.
  //
