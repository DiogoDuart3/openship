...
// here (from the pre-0.4.8 WIP squash commit) with no matching export in
// mail.controller.ts - dead since it was added, would 500 if ever hit, and
// nothing in the dashboard calls it (endpoints.ts has the string constant,
// unused). Removed rather than stubbed; a real "recheck port 25" action
// would need its own handler + dashboard wiring, out of scope here.
